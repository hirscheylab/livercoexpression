...
sortPear <- slice(sortPear , 1:length(sortPear$SIRT5)-1) #This removes the SIRT5 gene since that's irrelevent data
...
<<<<<<< HEAD
p1 <- dotplot(keggX, showCategory = 30, x = 'GeneRatio' , color = "pvalue", font.size = 12) 
#+ ggtitle("dotplot for SIRT5cor_all Data") #Creates a dotplot using the 30 processes with the lowest pvalues
plot_grid(p1, ncol=1)

cnetplot(keggX, foldChange=genes, node_label = TRUE, showCategory = 3)#Creates a network using the 5 processes with the lowest pvalues. The processes are central nodes that are connected to the genes that are in its respective gene set
=======
...
>>>>>>> 4a029ec6c01405fac845ae51cd780437002f6af8
